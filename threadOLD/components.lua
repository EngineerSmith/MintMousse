...
local loggerComponents = mintmousse._loggerComponents
...
// NEW
This is no longer the case; if the client receives action, and field e.g. "update", "size"
  it will reconstruct, and find what function it should call e.g. `update_child_size`, or `update_size`.
  This moves a lot of simple processing to the client, freeing up the server performance.

]]

local functionPattern = "%s*function%s+"
...
components.init = function()
  ...
  -- todo; we should review what data we're actually using outside and only 
  --       send what is needed than a copy of everything
  local channel = love.thread.getChannel(love.mintmousse.READONLY_BASIC_TYPES_ID)
  channel:performAtomic(function()
    channel:clear()
    channel:push(components.componentTypes) -- todo encode this message
  end)
  ...
  -- todo new controller needs a new way to access componentTypes than this old cheaty hack
...
components.parseComponentsMustache = function(components)
  for componentTypeName, componentType in pairs(components) do
    ...
    if path then
      local script, errorMessage = lfs.read(path)
      if not script then
        loggerComponents:warning("Unable to read HTML/Mustache file:", path, ". Reason:", errorMessage)
      else
...
components.logStats = function(components)
  ...
  loggerComponents:info("Found", count, "component types, with a total of", variable, "values that can be updated live.")
end
